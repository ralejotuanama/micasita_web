...
class FusionSC_Blog {
	...
	function post_meta_data( $return_all_meta = false ) {
		...
		if( self::$args['meta_author'] ) {
			$inner_content .= sprintf( '%s <span %s><a %s>%s</a>' . '</span><span %s>|</span>', __( 'Por', 'fusion-core' ), FusionCore_Plugin::attributes( 'blog-shortcode-meta-author' ), 
		...
		if( $return_all_meta ) {
			...
			if( self::$args['meta_comments'] ) {
				...
				ob_start();
				comments_popup_link( __( '0 Comentarios', 'fusion-core' ), __( '1 Comentario', 'fusion-core' ), '% ' . __( 'Comentarios', 'fusion-core' ) );
	...
	function read_more() {
		...
		if( self::$args['meta_link'] ) {
			...
			if( self::$args['meta_read'] ) {
				...
				$inner_content .= sprintf( '<a href="%s">%s</a>', get_permalink(),  __( 'Más información', 'fusion-core' ) );
				$inner_content .= '</p>';
