...
function themefusion_breadcrumb() {
		...
		echo '<ul class="breadcrumbs">';
		 ...
		 if ( !is_front_page() ) {
		echo '<li>'.$smof_data['breacrumb_prefix'].' <a style="color:#fff;  font-size: 13px;" href="';
		echo home_url();
		echo '">'.__('miCasita', 'Avada');
		echo "</a></li>";
		}
		...
		if(is_singular('avada_portfolio')) {
			...
			echo '<li style="color:#fff;  font-size: 13px;" href=">'.get_the_title().'</li>';
		}
		...
		if( is_404() ) { 
			if( class_exists( 'TribeEvents' ) && 
				...
				echo '<li>'.__("404 - Página no encontrada", 'Avada').'</li>'; 
			}
		}		
...
function tf_get_forum_pagination_links() {
	...
	$pagination_links = str_replace( '&larr;', __('Anterior', 'Avada').'<span class="page-prev"></span>', $pagination_links );
	$pagination_links = str_replace( '&rarr;', __('Siguiente', 'Avada').'<span class="page-next"></span>', $pagination_links );
		...
		<div class="page-title">
			<div class="page-title-wrapper">
				<div class="page-title-captions">
					<?php if( $title ): ?>
						<span style="color:#fff;  font-size: 13px;  float: left;"<?php if( ! $smof_data['disable_date_rich_snippet_pages'] ) { echo ' class="entry-title"'; } ?>><?php echo $title; ?> > </span>
						<?php if( $subtitle ): ?>
...
if( ! function_exists( 'avada_current_page_title_bar' ) ) {
	function avada_current_page_title_bar( $post_id ) {
		 		...
		 		( $smof_data['page_title_bar_bs'] == 'Breadcrumbs' && ( get_post_meta($post_id, 'pyre_page_title_breadcrumbs_search_bar', true) == 'default' || get_post_meta($post_id, 'pyre_page_title_breadcrumbs_search_bar', true) == '' ) ) ) {
					if( ( class_exists( 'Woocommerce' ) && is_woocommerce() ) || ( is_tax( 'product_cat' ) || is_tax( 'product_tag' ) ) ) {
						woocommerce_breadcrumb(array(
							'wrap_before' => '<ul class="breadcrumbs" style="color:#fff;  font-size: 13px;" >',
							'wrap_after' => '</ul>',
							'before' => '<li>',
							'after' => '</li>',
							'delimiter' => ''
						...
						bbp_breadcrumb( array ( 'before' => '<ul class="breadcrumbs" style="color:#fff;  font-size: 13px;">', 'after' => '</ul>', 'sep' => ' ', 'crumb_before' => '<li>', 'crumb_after' => '</li>', 'home_text' => __('miCasita', 'Avada')) );
					} else {
		...
		if( ! $title ) {
			...
			if( is_search() ) {
				$title = __('Resultados de la búsqueda:', 'Avada') . get_search_query();
			}
