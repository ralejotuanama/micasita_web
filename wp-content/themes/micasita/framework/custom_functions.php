		...
		if( ( $smof_data['post_meta'] && get_post_meta(get_queried_object_id(), 'pyre_post_meta', true) != 'no' ) || 
			...
			if( ( $layout == 'alternate' || $layout == 'grid_timeline' ) &&
			 ...
			 if( ! $smof_data['post_meta_author'] ) { 
				...
				if( $smof_data['disable_date_rich_snippet_pages'] ) {
					$metadata .= sprintf( '%s <span>%s</span><span class="sep">|</span>', __('Autor', 'Avada'), $author_post_link );
				} else {
					$metadata .= sprintf( '%s <span class="vcard"><span class="fn">%s</span></span><span class="sep">|</span>', __('Autor', 'Avada'), $author_post_link );
				}
			 } else {
			 ...
			 if( $layout != 'grid_timeline' ) {
				 ...
				 if( ! $smof_data['post_meta_comments'] ) {	
					ob_start();
					comments_popup_link( __('0 Comentarios', 'Avada'), __('1 Comentario', 'Avada'), '% ' . __('Comentarios', 'Avada') );
