<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'aProduto')){ ?>
    <a href="<?php echo base_url();?>index.php/productos/adicionar" class="btn btn-success"><i class="icon-plus icon-white"></i> Agregar Producto</a>
<?php } ?>
...
<table class="table table-bordered ">
    ...
    <tbody>
        <?php foreach ($results as $r) {
            echo '<tr>';
           /* echo '<td style=text-align:center;>'.$r->idProductos.'</td>';*/
            echo '<td style=text-align:center;>'.$r->descripcion.'</td>';
          /*  echo '<td>'.$r->stock.'</td>';
            echo '<td>'.number_format($r->precioVenta,2,',','.').'</td>';*/
           ...
           /* if($this->permission->checkPermission($this->session->userdata('permissao'),'dProduto')){
                echo '<a href="#modal-excluir" role="button" data-toggle="modal" produto="'.$r->idProductos.'" class="btn btn-danger tip-top" title="Eliminar Producto"><i class="icon-remove icon-white"></i></a>'; 
            }*/
