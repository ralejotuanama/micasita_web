...
class Mapos_model extends CI_Model {
    ...
    public function alterarSenha($clave,$oldSenha,$id){

        $this->db->where('idUsuarios', $id);
        $this->db->limit(1);
        $usuario = $this->db->get('usuarios')->row();

        if($usuario->clave != $oldSenha){
            return false;
        }
        else{
    ...
    function pesquisar($termo){
    	 ...
    	 $this->db->like('nombreCliente', $termo);
    	 ...
    	 $this->db->or_like('telefono', $termo);
    	 $this->db->limit(5);
         ...
         $this->db->or_like('descripcion',$termo);
         // buscando Precio Venta produtos
         $this->db->or_like('precioVenta',$termo);
         $this->db->limit(5);
         $data['produtos'] = $this->db->get('produtos')->result();

         //buscando id serviços
         $this->db->like('idServicios',$termo);
         //buscando Nombre serviços
         $this->db->or_like('nombre',$termo);
         //buscando Precio serviços
         $this->db->or_like('precio',$termo);
         $this->db->limit(5);
    ...
    function getOsAbertas(){
        $this->db->select('os.*, clientes.nombreCliente');
        $this->db->from('os');
    ...
    function getProdutosMinimo(){
        ...
        $sql = "SELECT * FROM produtos WHERE stock >= stockMinimo LIMIT 10";
