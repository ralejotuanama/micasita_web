...
class Financiero extends CI_Controller {
	...
	public function movimientos(){
        ...
        $config['base_url'] = base_url().'financiero/movimientos';
        $config['total_rows'] = $this->financeiro_model->count('lancamentos');
        $config['per_page'] = 100;
        $config['next_link'] = 'Proxima';
	    ...
	    $this->data['view'] = 'financiero/movimientos';
    ...
    function adicionargasto() {
        ...
        if(!$this->permission->checkPermission($this->session->userdata('permissao'),'aLancamento')){
           $this->session->set_flashdata('error','No tiene permiso para agregar un movimientos financieros.');
           redirect(base_url());
        }
        ...
        $this->data['custom_error'] = '';
        $urlAtual = $this->input->post('urlAtual');
        if ($this->form_validation->run('gasto') == false) {
            $this->data['custom_error'] = (validation_errors() ? '<div class="form_error">' . validation_errors() . '</div>' : false);
        } else {
    ...
    public function excluirLancamento(){   
        ...
        if(!$this->permission->checkPermission($this->session->userdata('permissao'),'dLancamento')){
           $this->session->set_flashdata('error','No tiene permisos para eliminar un movimientos financieros.');
           redirect(base_url());
        }
